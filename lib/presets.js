...
function wrapOptionFactory(presetOption) {
    return function OptionFactory(inputOption = {}, constructor) {
        const appliedOption = mergeOption(presetOption, inputOption);
...
function createPresetPage(...presetOptions) {
    const preparedFactories = presetOptions.map(option => {
        return isFactory(option) ? option : wrapOptionFactory(option);
    });
