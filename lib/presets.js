...
function mergeOption(presetOption, inputOption) {
    ...
    if (isBehavior(presetOption)) {
        inputOption.behaviors = _concat([presetOption], inputOption.behaviors);
        return inputOption;
    } else {
...
function createPresetPage(...presetOptions) {
    return function (appliedOption = {}) {
        const firstOption = presetOptions[0];
        if (isFactory(firstOption)) {
            firstOption(appliedOption, Page);
        } else {
            let mergedOption = {};
            for (const presetOption of presetOptions.reverse()) {
                mergedOption = mergeOption(presetOption, mergedOption);
            }
            appliedOption = mergeOption(mergedOption, appliedOption);
            Page(appliedOption);
        }
    }
}
